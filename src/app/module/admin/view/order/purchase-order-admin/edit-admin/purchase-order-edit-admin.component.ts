...
@Component({
  selector: 'app-purchase-order-edit-admin',
  templateUrl: './purchase-order-edit-admin.component.html'
})
export class PurchaseOrderEditAdminComponent extends AbstractEditController<PurchaseOrderDto, PurchaseOrderCriteria, PurchaseOrderService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.dateOrderSupplier = this.purchaseOrderService.format(this.item.dateOrderSupplier);
    }



    public validatePurchaseOrderProducts(){
   ...
   public addPurchaseOrderProducts() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.PurchaseOrderProductsElement.id == null){
                this.item.purchaseOrderProducts.push(this.PurchaseOrderProductsElement);
            }else{
                const index = this.item.purchaseOrderProducts.findIndex(e => e.id == this.PurchaseOrderProductsElement.id);
                this.item.purchaseOrderProducts[index] = this.PurchaseOrderProductsElement;
            }

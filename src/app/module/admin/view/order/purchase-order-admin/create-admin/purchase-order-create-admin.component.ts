...
export class PurchaseOrderCreateAdminComponent extends AbstractCreateController<PurchaseOrderDto, PurchaseOrderCriteria, PurchaseOrderService>  implements OnInit {
   ...
   private _validPurchaseOrderRefrence = true;
    ...
    private calculerTotal(item: PurchaseOrderDto) {
        var purchaseOrderProducts = item.purchaseOrderProducts;
        let total=0;
        for (let purchaseOrderProduct of purchaseOrderProducts) {
            total+=purchaseOrderProduct.price*purchaseOrderProduct.qantity;
        }
        item.total=total;
